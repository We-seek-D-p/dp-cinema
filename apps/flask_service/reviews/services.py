...
from .errors import ConflictError, NotFoundError, DependencyError, ForbiddenError, ValidationError
...
def _verify_movie_exists(movie_id: int) -> bool:
    ...
    try:
        ...
        raise DependencyError(f"Не удалось связаться с Django: {str(e)}")
    ...
    try:
        with httpx.Client(timeout=2.0) as client:
            ...
            if resp.status_code not in (200, 201, 204):
                raise DependencyError(f"Не удалось отправить запрос на модерацию: {resp.status_code}")
    except Exception as e:
        raise DependencyError(f"Ошибка подключения во время запроса на модерацию: {str(e)}")
...
class ReviewService:
    ...
    def create_review(self, user_id: int, movie_id: int, text: str, rating: int) -> Review:
    ...
    def update_review(self, review_id: int, user_id: int, text: str, rating: int) -> Review:
